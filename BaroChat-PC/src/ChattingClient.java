...
public class ChattingClient { //클라이언트를 위한 클레스
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        DataOutputStream printChat = null; //입력된 채팅을 출력하는 용도

        InetAddress myIp = InetAddress.getByName("172.20.10.10"); //IP
        ...
        /*
        // 인원수
        pNum = new JLabel("0명", SwingConstants.RIGHT);
        pNum.setBounds(285, 5, 60, 25);
        pNum.setForeground(new Color(58, 40, 109));
        pNum.setFont(f2);
        p1.add(pNum);*/
        ...
        /*
        //IP주소 입력창
        String iP= JOptionPane.showInputDialog(null,"서버IP를 입력하세요","10.101.13.169"); //IP를 메세지출력창을 통해 입력받음
        if(iP==null || iP.length()==0){  //주소입력되지않거나, 0이면 안전하게 종료시킴
            System.out.println("서버 IP가 입력되지 않았습니다.");
            System.exit(0);
        }*/
        //포트번호 입력창
        String aptn = JOptionPane.showInputDialog(null,"거주 중인 동을 입력하세요.","1"); //포트번호를 메세지출력창을 통해 입력받음
        int aptNum =  Integer.parseInt(aptn); //문자열로 받았으므로 숫자로 형변환해줌
        int portn = 0;
        if (aptNum <= 4 && aptNum >=1) { //입력한 동이 올바르지 않으면 안전하게 종료시킴
            if(aptNum == 1) {
                portn = 9996;
            } else if (aptNum == 2) {
                portn = 9997;
            ...
            System.out.println("입력하신 동이 올바르지 않습니다.");
            System.exit(0);
        }
        ...
        try {
            ...
            sock = new Socket(myIp, portn); //ip주소와 메세지출력창에서 받은 포트번호로 소켓에 접근
